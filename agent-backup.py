...
# --- Recorder Settings ---
RECORDER_INTERACTION_TIMEOUT = 10.0 # Seconds to wait for user override/confirmation
...
class WebAgent:
    ...
    AI-assisted web test recording or executes pre-planned steps.
    In RECORDER mode, it interacts with the user to confirm/override AI suggestions.
    """
    ...
    def __init__(self,
                 ...
                 max_retries_per_subtask: int = 3, # Max retries per *failed* step
                 max_extracted_data_history: int = 7,
                 is_recorder_mode: bool = False): # History for verification

        # --- Check for recorder mode constraint ---
        if is_recorder_mode and headless:
             logger.warning("Recorder mode initiated but headless is True. Suggesting headless=False for interactive recording.")
        # --- End check ---
        ...
        mode_string = "RECORDER" if self.is_recorder_mode else "EXECUTOR (Direct - Deprecated in Agent)"
        logger.info(f"WebAgent initialized in {mode_string} Mode (headless={headless}, max_iter={max_iterations}, max_retries={max_retries_per_subtask}).")
    ...
    def _plan_subtasks(self, feature_description: str):
        """Uses the LLM to break down the feature test into sequential, verifiable steps FOR RECORDING."""

        # --- RECORDER MODE ---
        # Prompt is slightly different: focus on user actions and verifiable states.
        # LLM doesn't need to worry about execution details as much, just the logical flow.
        logger.info(f"[Recorder] Planning test steps for feature: '{feature_description}'")
        self.feature_description = feature_description
        self.recorded_steps = [] # Reset recorded steps


        # --- Prompt Construction ---
        prompt = f"""
        You are an AI Test Engineer RECORDING an automated test script.
        Given the feature description: "{feature_description}"


        Break this down into a sequence of specific, verifiable browser interaction steps that an automated web agent can perform.
        Each step must be a single, clear instruction or verification (e.g., "Navigate to...", "Click the button [index] with text 'Submit'", "Extract text from element [index] with id 'price'"). The user can confirm or override each step during recording. But don't rely on the user
        ...
        **Key Elements of Recording plan:**
