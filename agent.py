...
3.  Respond with a JSON object matching the required schema.
    ...
    *   **If `verified` is TRUE:**
        ...
        *   **`verification_selector` (Required):** Provide a robust CSS selector for the explicitely verifying element. If the element is static and has a parent hint like `(inside: <div id="parent-id">)`, try to incorporate the parent into the selector (e.g., `#parent-id > span.label`). Use attributes (id, data-testid, name, class) or specific text content shown in the context.
...
{{
  ...
  "reasoning": "Verification step asked for 'You logged into...', but the static <p class='success-message' (Static)> element shows the success message 'signed in...', confirming the login intent."
}}
```
